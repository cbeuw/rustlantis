...
impl<T> RangeMap<T> {
    ...
    fn find_offset(&self, offset: u64) -> usize {
        ...
        loop {
            debug_assert!(
                left < right,
                "find_offset: offset {offset} is out-of-bounds"
            );
                ...
                if successful_merge_count > 0 {
                    if done || self.v[end_idx].data != self.v[equal_since_idx].data {
                        ...
                        if removed_elems > 0 {
                            ...
                            self.v
                                .splice(equal_since_idx + 1..end_idx, std::iter::empty());
...
mod tests {
    ...
    fn to_vec<T: Copy>(map: &RangeMap<T>, offset: u64, len: u64) -> Vec<T> {
        (offset..offset + len)
            .map(|i| {
                map.iter(Size::from_bytes(i), Size::from_bytes(1))
                    .next()
                    .map(|(_, &t)| t)
                    .unwrap()
            })
